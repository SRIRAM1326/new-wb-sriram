    ...
    return (
        <nav
            className={cn(
                ...
                scrolled ? "glass-card py-3" : "bg-transparent"
            )}
        >
                ...
                <div className="hidden md:flex items-center gap-8">
                    {navLinks.map((link) => (
                        <Link
                            key={link.name}
                            href={link.href}
                            className="text-sm font-medium text-text-body hover:text-primary-cyan transition-colors"
                        >
                            {link.name}
                        </Link>
                    ))}
                    <Link
                        href="/resume.pdf"
                        target="_blank"
                        className="px-5 py-2 rounded-full button-space text-sm font-bold transition-all transform hover:scale-105"
                    >
                        Resume
                    </Link>
                </div>
            ...
            {mobileMenuOpen && (
                <motion.div
                    ...
                    className="absolute top-full left-0 right-0 glass-card border-t border-border-main py-6 px-6 md:hidden flex flex-col gap-4"
                >
                    {navLinks.map((link) => (
                        <Link
                            key={link.name}
